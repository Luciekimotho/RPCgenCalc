...
calculator_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	case addition:
		_xdr_argument = (xdrproc_t) xdr_my_numbers;
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, CALCULATOR, CALC, calculator_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (CALCULATOR, CALC, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, CALCULATOR, CALC, calculator_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (CALCULATOR, CALC, tcp).");
		exit(1);
	}
